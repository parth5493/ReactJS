...
const BirthdayMainPagewithFunctional = () => {
    let [name, setName] = useState(people)
    let PeopleList = name.map((list, index) => {
    ...
    return (
        <>
            <div className="container container_color square border border-info w-50 p-3 square rounded mt-5">
                    ...
                    <button className='btn_width' onClick={()=>setName([])}>Clear all</button>
                </div>
        </>
    );
};

export default BirthdayMainPagewithFunctional;